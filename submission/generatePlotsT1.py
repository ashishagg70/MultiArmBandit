...
plt.plot(logKeys, [ np.array(ep2[x]).mean() for x in keys], label='eplison-greedy',color='lightblue', linewidth=2)
plt.plot(logKeys, [ np.array(ucb2[x]).mean() for x in keys],label='ucb', color='red', linewidth=2)
plt.plot(logKeys, [ np.array(klUcb2[x]).mean() for x in keys], label='kl-ucb',color='orange', linewidth=2)
plt.plot(logKeys, [ np.array(ts2[x]).mean() for x in keys], label='thompson-sampling',color='green', linewidth=2)
plt.legend(loc='upper left')
plt.show()
plt.clf()
...
plt.plot(logKeys, [ np.array(ep3[x]).mean() for x in keys], label='eplison-greedy',color='lightblue', linewidth=2)
plt.plot(logKeys, [ np.array(ucb3[x]).mean() for x in keys],label='ucb', color='red', linewidth=2)
plt.plot(logKeys, [ np.array(klUcb3[x]).mean() for x in keys], label='kl-ucb',color='orange', linewidth=2)
plt.plot(logKeys, [ np.array(ts3[x]).mean() for x in keys], label='thompson-sampling',color='green', linewidth=2)
plt.legend(loc='upper left')
plt.show()
plt.clf()
