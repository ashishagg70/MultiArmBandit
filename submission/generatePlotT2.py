...
for line in open('outputDataT2_me.txt', 'r'):
...
plt.plot(logKeys, [ np.array(tsh1[x]).mean() for x in keys], label='thompson-sampling-with-hint',color='lightblue', linewidth=2)
...
plt.plot(logKeys, [ np.array(tsh2[x]).mean() for x in keys], label='thompson-sampling-with-hint',color='lightblue', linewidth=2)
...
plt.plot(logKeys, [ np.array(tsh3[x]).mean() for x in keys], label='thompson-sampling-with-hint',color='lightblue', linewidth=2)
